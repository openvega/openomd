...
#pragma clang diagnostic pop
    ...
    public:
        ...
        uint16_t side() const
        {
            return m_dimensions.side();
        }

        char bqMoreFlag() const
        {
            return m_dimensions.blockLength();
        }
        // SBE_OMD_HACK
    ...
    #pragma clang diagnostic pop
        ...
        Items &item(const std::uint16_t value)
        {
            *((std::uint16_t *)(m_buffer + m_offset + 0)) = SBE_LITTLE_ENDIAN_ENCODE_16(value);
            return *this;
        }

        static SBE_CONSTEXPR std::uint16_t itemTypeId() SBE_NOEXCEPT
        {
            return 104;
        }

        static SBE_CONSTEXPR std::uint64_t itemTypeSinceVersion() SBE_NOEXCEPT
        {
             return 0;
        }

        bool itemTypeInActingVersion() SBE_NOEXCEPT
        {
    #if defined(__clang__)
    #pragma clang diagnostic push
    #pragma clang diagnostic ignored "-Wtautological-compare"
    #endif
            return m_actingVersion >= itemTypeSinceVersion();
    #if defined(__clang__)
    #pragma clang diagnostic pop
    #endif
        }

        static SBE_CONSTEXPR std::size_t itemTypeEncodingOffset() SBE_NOEXCEPT
        {
             return 2;
        }


        static const char *itemTypeMetaAttribute(const MetaAttribute::Attribute metaAttribute) SBE_NOEXCEPT
        {
            switch (metaAttribute)
            {
                case MetaAttribute::EPOCH: return "unix";
        ...
        static SBE_CONSTEXPR char itemTypeNullValue() SBE_NOEXCEPT
        {
            return (char)0;
        }

        static SBE_CONSTEXPR char itemTypeMinValue() SBE_NOEXCEPT
        {
            return (char)32;
        }

        static SBE_CONSTEXPR char itemTypeMaxValue() SBE_NOEXCEPT
        {
            return (char)126;
        }

        static SBE_CONSTEXPR std::size_t itemTypeEncodingLength() SBE_NOEXCEPT
        {
            return 1;
        }
        ...
        Items &itemType(const char value)
        {
    ...
    #pragma clang diagnostic pop
        ...
        Items &filler(const std::uint8_t value)
        {
...
public:
    ...
    static SBE_CONSTEXPR std::uint16_t itemsId() SBE_NOEXCEPT
    {
        return 102;
    }
    ...
    inline Items &items()
    {
        m_items.wrapForDecode(m_buffer, m_positionPtr, m_actingVersion, m_bufferLength);
        return m_items;
    }

    Items &itemsCount(const std::uint16_t count)
    {
        m_items.wrapForEncode(m_buffer, count, m_positionPtr, m_actingVersion, m_bufferLength);
        return m_items;
    }

    static SBE_CONSTEXPR std::uint64_t itemsSinceVersion() SBE_NOEXCEPT
    {
         return 0;
    }

    bool itemsInActingVersion() SBE_NOEXCEPT
    {
#if defined(__clang__)
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wtautological-compare"
#endif
        return m_actingVersion >= itemsSinceVersion();
#if defined(__clang__)
#pragma clang diagnostic pop
#endif
    }
