...
#ifndef _OMDC_SBE_GROUPSIZE16_H_
#define _OMDC_SBE_GROUPSIZE16_H_
...
public:
    GroupSize16() : m_buffer(nullptr), m_offset(0) {}

    GroupSize16(char *buffer, const std::uint64_t bufferLength, const std::uint64_t actingVersion)
    {
        reset(buffer, 0, bufferLength, actingVersion);
    }

    GroupSize16(const GroupSize16& codec) :
...
#if __cplusplus >= 201103L
    GroupSize16(GroupSize16&& codec) :
        m_buffer(codec.m_buffer),
        m_bufferLength(codec.m_bufferLength),
        m_offset(codec.m_offset),
        m_actingVersion(codec.m_actingVersion){}

    GroupSize16& operator=(GroupSize16&& codec) SBE_NOEXCEPT
    {
        m_buffer = codec.m_buffer;
        m_bufferLength = codec.m_bufferLength;
        m_offset = codec.m_offset;
    ...
    GroupSize16& operator=(const GroupSize16& codec) SBE_NOEXCEPT
    {
        m_buffer = codec.m_buffer;
        m_bufferLength = codec.m_bufferLength;
        m_offset = codec.m_offset;
    ...
    GroupSize16 &wrap(char *buffer, const std::uint64_t offset, const std::uint64_t actingVersion, const std::uint64_t bufferLength)
    {
    ...
    GroupSize16 &numInGroup(const std::uint16_t value)
    {
    ...
    GroupSize16 &blockLength(const std::uint16_t value)
    {
